...
public:
    ...
    std::string resolveSaveDirectory(const std::string& destination) const
    {
        const char* home_env = std::getenv("HOME");
        const std::string home = home_env == nullptr ? std::string() : std::string(home_env);

        if (destination.empty())
        {
            if (savePCDDirectory.empty())
                return std::string();
            if (!home.empty() && savePCDDirectory.front() == '/')
                return home + savePCDDirectory;
            if (!home.empty())
                return home + "/" + savePCDDirectory;
            return savePCDDirectory;
        }
    ...
    bool ensureDirectoryExists(const std::string& directory) const
    {
        ...
        {
            size_t slash = directory.find('/', index);
            std::string part = directory.substr(index, slash - index);
            index = slash == std::string::npos ? directory.size() : slash + 1;
    ...
    bool saveGlobalMapToDirectory(const std::string& saveMapDirectory, float resolution, bool binary)
    {
        if (cloudKeyPoses3D->empty() || cloudKeyPoses6D->empty())
        {
            RCLCPP_WARN(get_logger(), "No LIO-SAM keyframes are available. Move the robot before saving a map.");
            return false;
        }

        const size_t keyframe_count = cloudKeyPoses3D->size();
        if (cloudKeyPoses6D->size() < keyframe_count ||
            cornerCloudKeyFrames.size() < keyframe_count ||
            surfCloudKeyFrames.size() < keyframe_count)
        {
            RCLCPP_ERROR(get_logger(), "Map keyframe buffers are inconsistent, skipping map save.");
            return false;
        }

        auto save_cloud = [binary](const std::string& path, const auto& cloud) {
            return binary ? pcl::io::savePCDFileBinary(path, cloud)
                          : pcl::io::savePCDFileASCII(path, cloud);
        };

        int ret = 0;
        ret |= save_cloud(saveMapDirectory + "/trajectory.pcd", *cloudKeyPoses3D);
        ret |= save_cloud(saveMapDirectory + "/transformations.pcd", *cloudKeyPoses6D);
        ...
        for (size_t i = 0; i < keyframe_count; i++)
        {
            ...
            *globalSurfCloud += *transformPointCloud(surfCloudKeyFrames[i], &cloudKeyPoses6D->points[i]);
            cout << "\r" << std::flush << "Processing feature cloud " << i << " of " << keyframe_count << " ...";
        }
        ...
        if (resolution > 0.0f)
        {
            ...
            ret |= save_cloud(saveMapDirectory + "/CornerMap.pcd", *globalCornerCloudDS);
            ret |= save_cloud(saveMapDirectory + "/SurfMap.pcd", *globalSurfCloudDS);
            *globalMapCloud += *globalCornerCloudDS;
            *globalMapCloud += *globalSurfCloudDS;
        }
        else
        {
            ret |= save_cloud(saveMapDirectory + "/CornerMap.pcd", *globalCornerCloud);
            ret |= save_cloud(saveMapDirectory + "/SurfMap.pcd", *globalSurfCloud);
            *globalMapCloud += *globalCornerCloud;
            *globalMapCloud += *globalSurfCloud;
        }

        ret |= save_cloud(saveMapDirectory + "/GlobalMap.pcd", *globalMapCloud);
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_mapOptimization", options)
    {
        ...
        auto saveMapService = [this](const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<lio_sam::srv::SaveMap::Request> req, std::shared_ptr<lio_sam::srv::SaveMap::Response> res) -> void {
            ...
            const string saveMapDirectory = resolveSaveDirectory(req->destination);
            cout << "Save destination: " << saveMapDirectory << endl;

            if (!ensureDirectoryExists(saveMapDirectory))
            {
                RCLCPP_ERROR(get_logger(), "Failed to create map save directory: %s", saveMapDirectory.c_str());
                res->success = false;
                return;
            }

            {
                ...
                res->success = saveGlobalMapToDirectory(saveMapDirectory, req->resolution, true);
            }

            cout << "****************************************************" << endl;
            cout << (res->success ? "Saving map to pcd files completed\n" : "Saving map to pcd files failed\n") << endl;
            return;
        };
    ...
    void visualizeGlobalMapThread()
    {
        ...
        cout << "****************************************************" << endl;
        cout << "Saving map to pcd files ..." << endl;
        const string saveMapDirectory = resolveSaveDirectory("");
        cout << "Save destination: " << saveMapDirectory << endl;
        bool success = false;
        if (ensureDirectoryExists(saveMapDirectory))
        {
            std::lock_guard<std::mutex> lock(mtx);
            success = saveGlobalMapToDirectory(saveMapDirectory, 0.0f, false);
        }
        else
        {
            RCLCPP_ERROR(get_logger(), "Failed to create map save directory: %s", saveMapDirectory.c_str());
        ...
        cout << "****************************************************" << endl;
        cout << (success ? "Saving map to pcd files completed" : "Saving map to pcd files failed") << endl;
    }
